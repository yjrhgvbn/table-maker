...
export function DsTable(properties: DsTableProperties) {
  ...
  const createNewData = useCallback(() => {
    ...
    const typeDeafultValueMap = {
      ...
      [ColumnConfigType.Select]: FieldType.String,
    };
    for (const { accessorKey, type } of columnConfigs) {
      if (accessorKey in result) {
  ...
  const columns = useMemo<MRT_ColumnDef<DsField>[]>(() => columnConfigs.map<MRT_ColumnDef<DsField>>((column) => {
